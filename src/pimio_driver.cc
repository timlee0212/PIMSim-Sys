...
int PIMIODriver::open(ThreadContext *tc, int mode, int flags) {
...
int PIMIODriver::ioctl(ThreadContext *tc, unsigned req, Addr ioc_buf) {
...
Addr PIMIODriver::mmap(ThreadContext *tc, Addr start, uint64_t length, int prot,
                       int tgt_flags, int tgt_fd, off_t offset) {
    panic("mmap function for PIM function is not implemented.");
}

void PIMIODriver::signalWakeup() {
    assert(sleep_tc);
    sleep_tc->activate();
    sleep_tc = nullptr;
}
